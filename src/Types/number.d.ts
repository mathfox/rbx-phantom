// source: https://github.com/KamilHs/type-samurai/blob/79490c1d7eb919761b6d70683f67f0588614841b/src/number.d.ts
...
export type Odd<T extends number> = IfNot<IsNever<Integer<T>>, If<IsNever<Even<T>>, T, never>, never>;

export type IsInteger<T extends number> = Not<IsNever<Integer<T>>>;

export type IsFloat<T extends number> = Not<IsNever<Float<T>>>;

export type IsEven<T extends number> = If<IsInteger<T>, `${T}` extends `${string}${EvenDigit}` ? true : false>;
...
export type IfInteger<T extends number, IfTrue = true, IfFalse = false> = If<IsInteger<T>, IfTrue, IfFalse>;

export type IfFloat<T extends number, IfTrue = true, IfFalse = false> = If<IsFloat<T>, IfTrue, IfFalse>;

export type IfEven<T extends number, IfTrue = true, IfFalse = false> = If<IsEven<T>, IfTrue, IfFalse>;

export type IfOdd<T extends number, IfTrue = true, IfFalse = false> = If<IsOdd<T>, IfTrue, IfFalse>;

export type IfPositive<T extends number, IfTrue = true, IfFalse = false> = If<IsPositive<T>, IfTrue, IfFalse>;

export type IfNegative<T extends number, IfTrue = true, IfFalse = false> = If<IsNegative<T>, IfTrue, IfFalse>;
...
export type IfPositiveFloat<T extends number, IfTrue = true, IfFalse = false> = If<IsPositiveFloat<T>, IfTrue, IfFalse>;

export type IfNegativeFloat<T extends number, IfTrue = true, IfFalse = false> = If<IsNegativeFloat<T>, IfTrue, IfFalse>;

export type ParseNumber<T extends string | number> = T extends `${infer NumT extends number}` ? NumT : never;

export type Abs<T extends number> = `${T}` extends `-${infer PositiveT extends number}` ? PositiveT : T;
